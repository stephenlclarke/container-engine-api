...
The executable and provider-session boundary are implemented, but the stock devcontainer adapter and enhanced Container authority have not both completed their process cutover. Explicit drain/import/commit handoff, provider-session authentication beyond current-user private socket ownership, request-body streaming/spooling, and the complete route handlers remain required before whole-API or `use_api_socket` compatibility can be advertised.
