...
The package currently exposes five libraries:
...
This package remains library-only and does not claim complete Docker Engine API 1.53 compatibility. The logging controller implements the shared logging surface but intentionally returns only logging-relevant `/info` and inspect fields; the adopting gateway must compose the remaining Engine fields and use the same authoritative backend. The runtime SPI now establishes exclusive provider identity, but the shared `container-engine` executable, provider-session transport, and generated complete route ledger remain to be implemented.
