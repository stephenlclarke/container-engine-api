...
func `public gateway relays websocket bytes through its provider session`() async throws {
    let root = URL(fileURLWithPath: "/tmp", isDirectory: true)
        .appendingPathComponent(
            "ceg-ws-\(UUID().uuidString.prefix(8))",
            isDirectory: true
        )
    ...
    let provider = try ContainerEngineProviderSessionServer(
        responder: GatewayWebSocketResponder(),
...
private struct GatewayWebSocketResponder: DockerHTTPResponder {
...
private final class GatewayUnixSocketClient {
