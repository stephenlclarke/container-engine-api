...
final class RawClientPeer: Sendable {
    ...
    init(response: String, trickle: Bool = false) throws {
        ...
        do {
            ...
            guard socketPath.utf8.count < MemoryLayout.size(ofValue: address.sun_path) else { throw POSIXError(.ENAMETOOLONG) }
        ...
        worker = Task {
            await withCheckedContinuation { continuation in
                DispatchQueue.global().async {
                    Self.serve(listener: listener, state: state, response: Data(response.utf8), trickle: trickle)
                    continuation.resume()
                }
            }
        }
    ...
    private static func serve(listener: Int32, state: RawPeerState, response: Data, trickle: Bool) {
        ...
        guard setsockopt(accepted, SOL_SOCKET, SO_NOSIGPIPE, &noSignal, socklen_t(MemoryLayout.size(ofValue: noSignal))) == 0,
              setsockopt(accepted, SOL_SOCKET, SO_SNDTIMEO, &timeout, socklen_t(MemoryLayout.size(ofValue: timeout))) == 0,
              setsockopt(accepted, SOL_SOCKET, SO_RCVTIMEO, &timeout, socklen_t(MemoryLayout.size(ofValue: timeout))) == 0,
        ...
        if trickle {
            while !state.isStopped, ContinuousClock.now < deadline {
                guard write(Data("x".utf8), descriptor: accepted) else { return }
