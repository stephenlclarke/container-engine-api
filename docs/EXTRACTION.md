...
The provider selection record rejects a declaration or provider-owned state-root change and requires an explicit handoff rather than silently selecting a fallback. `container-engine` now binds the public listener only after probing and selecting one out-of-process provider, and every provider request repeats the fingerprint-bound handshake. The private provider socket uses a mode-`0600` endpoint, non-followed single-link singleton lock, current-user private parent, and exact-inode cleanup. Protocol schema 3 forwards request bodies in ordered 1 MiB frames with an explicit end marker, then carries bytes, pull-based response streams, raw bidirectional hijacks, and WebSocket session identity without linking the provider's Apple packages into the gateway process. The shared Unix server performs RFC 6455 framing only at the public boundary, while the provider protocol continues to carry exact unframed session bytes.

Devcontainer supplies the stock adapter through the private provider session with an exact route capability declaration, and its normal production mode now routes its public socket through `ContainerEngineGatewayResponder` rather than directly to its provider router. The matched enhanced Container implementation supplies the logging authority adapter and declaration, but synchronized public dependency pins remain pending. The signed drain/import/commit/complete handoff is also not implemented. Therefore the package provides and tests the singular-gateway mechanism but does not yet prove that every client observes one migrated authority across a handoff.

### Bounded streaming

Each inbound request body is still materialized as one `Foundation.Data` value, subject to per-request, aggregate-buffer, and pending-request caps. The logging surface uses `DockerHTTPStreamSession`: the server pulls one chunk at a time, awaits the NIO write before requesting another, closes natural completion exactly once, and cancels the exact source when the connection is force-closed. Raw and WebSocket hijack output likewise await each channel write. The source-compatible `AsyncThrowingStream<Data>` response case still does not suspend its producer on NIO writability. Raw and WebSocket hijack input is bounded: each admitted event is at most 1 MiB, at most 16 events can wait behind the active write, and overflow cancels the exact session instead of silently dropping bytes. WebSocket aggregation is separately capped at 16 MiB. Large uploads still require incremental delivery or inode-safe private-file spooling, and adopters must use managed streams for large downloads.
