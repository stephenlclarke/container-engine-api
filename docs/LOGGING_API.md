...
Versionless routes use API 1.53 behavior. Versioned requests from 1.44 through 1.53 are accepted. A structurally known route with an older or newer version receives a Docker 29.2.1-shaped `400` envelope before query parsing or backend contact. An unknown method/path returns Docker's JSON `404 page not found` envelope before version rejection, including when its prefix is outside the supported range. `responseIfHandled(to:)` permits a complete gateway to compose non-logging controllers without duplicating version/query parsing.
...
The legacy generic `AsyncThrowingStream<Data>` response remains source-compatible but is not suitable for unbounded production downloads because it can enqueue writes faster than channel writability. Logging does not use it. Raw and WebSocket hijack output await every channel write. Public input uses a byte-accounted 16 MiB queue that coalesces normal fragmented Unix reads into at most 1 MiB writes. If the consumer cannot keep up beyond that bound, the server rejects the new data, closes the channel, and cancels the exact session rather than dropping or reordering bytes. The private provider boundary transfers ordinary request bodies in ordered 1 MiB frames with an explicit end marker and a 1 GiB total bound, matching the public server limit without weakening the 48 MiB per-frame guard. Hijack input uses at most 1 MiB frames and a 4 MiB ordered relay: writes and EOF retain wire order while backpressure can pause command intake without blocking concurrent output or wait handling. WebSocket messages are bounded to 16 MiB and fragmented input is aggregated only within that bound. With remote half-close enabled, a completely silent follow cannot observe whether a peer intended a full close or only closed its request-writing side until the next failed output write or a server drain fence; either boundary cancels the reader.
...
The immutable [v0.3.2 transport matrix](performance/engine-streaming-v0.3.2/timing-matrix.md) and its adjacent raw TSV, fingerprints, summary JSON, and JUnit evidence identify clean signed code head `aefa9cb13bceae5e0921faef055085d8976f3966`, the candidate binary digest, same-host Docker Engine, environment, and all 66 successful samples. Candidate/Docker medians were 0.44x for resize, 0.79x for a 32-byte WebSocket round trip, and 0.45x for a 1 MiB WebSocket round trip. Resize and 1 MiB WebSocket were directionally better, the sub-millisecond 32-byte WebSocket lane was comparable, and every gate passed.
